...
from waitlyst.events import TrackEvent, PageEvent, GroupEvent, ScreenEvent, AliasEvent
...
class IndexTestCase(unittest2.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertTrue(hasattr(self.waitlyst, "group"))
        self.assertTrue(hasattr(self.waitlyst, "alias"))
        self.assertTrue(hasattr(self.waitlyst, "screen"))
    ...
    def test_alias(self, post):
        ...
        # Assert that the alias event has been added to the queue
        self.waitlyst.alias("newId")
        self.assertEqual(len(self.waitlyst.queue), 2)
        self.assertIsInstance(self.waitlyst.queue[1], AliasEvent)
        self.assertEqual(self.waitlyst.queue[1].user_id, "newId")
    ...
    @patch("waitlyst.http.HttpClient.post", side_effect=mockClient.handle_post)
    def test_screen(self, post):
        """Test that screening events works."""
        self.waitlyst.reset()
        self.waitlyst.identify(self.user_id)
        self.waitlyst.screen("test_track")

        # Assert that event has been successfully added.
        self.assertEqual(len(self.waitlyst.queue), 2)
        event = self.waitlyst.queue[1]
        self.assertIsInstance(event, ScreenEvent)
        self.assertEqual(event.type, ScreenEvent.type)
    ...
    @patch("waitlyst.http.HttpClient.post", side_effect=mockClient.handle_post)
    def test_group(self, post):
        """Test that grouping events works."""
        self.waitlyst.reset()
        gid = str(uuid.uuid4())
        self.waitlyst.group(gid)

        # Assert that event has been successfully added.
        self.assertEqual(len(self.waitlyst.queue), 1)
        event = self.waitlyst.queue[0]
        self.assertIsInstance(event, GroupEvent)
        self.assertEqual(event.type, GroupEvent.type)
